...
class AcknowPodDecoderTests: XCTestCase {
    ...
    func testHeaderFooter() throws {
        ...
        let acknowList = try AcknowPodDecoder().decode(from: url)
    ...
    func testGeneralPerformance() throws {
        ...
        self.measure() {
            _ = try? AcknowPodDecoder().decode(from: url)
        }
    }

    func testParseNonExistentFile() {
        let url = URL(fileURLWithPath: "/404")
        XCTAssertThrowsError(try AcknowPodDecoder().decode(from: url))
    }
}
